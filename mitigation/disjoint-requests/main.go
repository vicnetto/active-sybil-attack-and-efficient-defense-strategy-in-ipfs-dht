...
func lookupForCid(flags Flags) {
	...
	for p := range clientNode.DHT.WAN.FindProvidersAsyncDisjoint(timeoutCtx, decodedCid, 10) {
		log.Info.Println("Found provider:", p)
		providers = append(providers, p.ID.String())

		if p.ID.String() == flags.providerPid {
			timeoutCancel()
		}
	}
	...
	// log.Info.Println("Peers queried:", dht.QueriedPeers)
}
