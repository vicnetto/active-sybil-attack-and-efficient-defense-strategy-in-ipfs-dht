...
var hydraName = "hydra-sybil"

const (
	defaultBucketSize         = 20
	defaultMetricsAddr        = "127.0.0.1:9758"
	defaultHTTPAPIAddr        = "127.0.0.1:7779"
	defaultConnMgrHighWater   = 20000
	defaultConnMgrLowWater    = 18000
...
func main() {
	...
	headsFilePath := flag.String("heads-file-path", "", "Specify the path to the heads file.")
	cid := flag.String("cid", "", "Specify the CID to eclipse.")
	isActive := flag.Bool("active", false, "Sybil should be active.")
	isPassive := flag.Bool("passive", false, "Sybil should be passive.")
	...
	name := flag.String("name", hydraName, "A name for the Hydra (for use in metrics)")
	...
	if len(*headsFilePath) == 0 {
		fmt.Println("🚩 -heads-file-path missing.")
		missingFlag = true
	}

	if len(*cid) == 0 {
		fmt.Println("🚩 -cid missing.")
		missingFlag = true
	}

	if (!*isActive && !*isPassive) || (*isActive == true && *isPassive == true) {
		fmt.Println("🚩 one mode should be specified (-active or -passive).")
		missingFlag = true
	}

	if missingFlag {
		fmt.Println()
		fmt.Println("./hydra-booster -help")
		os.Exit(1)
	}
	...
	headInformation := hydra.GetHeadsPrivateKey(*headsFilePath)
	*nheads = len(headInformation)
