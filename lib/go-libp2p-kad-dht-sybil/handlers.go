...
func (dht *IpfsDHT) handleGetProviders(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	...
	if EclipsedCid != cidCast.String() {
		...
		if IsActive {
			...
			for i := 0; i < dht.alpha; i++ {
				// Generating completely random peer and address
				genPid, _ := dht.RoutingTable().GenRandPeerID(0)
				ip := []multiaddr.Multiaddr{multiaddr.StringCast("/ip4/" + pb.GenerateRandomIp().String() + "/tcp/4001")}
			...
			fmt.Printf("[%s] ", time.Now().Format(time.RFC3339))
			fmt.Println(p.String(), "asked about", cidCast.String()+", ignoring...")
		}
	}
